...
def LSUniqueID(path,row,year,jd,directory):
	LandsatID=os.path.join(directory,"LC8" + str(path) + '0' + str(row)+str(year)+str(jd-11)+"LGN00")
	return(LandsatID)
...
def downloadLandsatScene(jd,year,month,day,path,row,directory):
	...
	# downLoadCommand=getCurrentDirectory() + "/download_landsat_scene.py -o scene -b LC8 -s 007203 -d 20140701"+' -s ' + path + '0'+str(row)+ " -u usgs.txt -p proxy.txt --output " + LandsatID
	...
	# print('extracting...')
	# u have to give name of file downloaded
	tarName=extractTar(LandsatID,os.path.join(directory,str(row)))
	# allFiles=glob.glob(os.path.join(directory,str(row),LandsatID,'*.TIF'))
	...
	# 		os.remove(filename)
	# try:
	# 	shutil.rmtree(os.path.join(directory,str(row),LandsatID,tarName))
	# except:
	# 	print('No folder to delete called: ' + os.path.join(directory,str(row),LandsatID,tarName))
	# try:
	# 	os.remove(os.path.join(directory,str(row),LandsatID,tarName + '_MTL.txt'))
	# except:
	# 	print('No file to delete called: ' + os.path.join(directory,str(row),LandsatID,tarName + '_MTL.txt'))
	for file in os.listdir(os.path.join(directory, str(row))):
		if file.endswith("_B8.TIF"):
			fn = os.path.join(directory, str(row), file[:-7])

	src = gdal.Open(fn + '_B8.TIF',gdalconst.GA_ReadOnly)
	match = gdal.Open(fn + '_B4.TIF',gdalconst.GA_ReadOnly)
	dst_filename = os.path.join(fn + '_B8.TIF')

	print("Reprojecting ========= ")
	reprojectPanBand(src, match, dst_filename)
	SLIP.model(year+month+day,path,str(row))	
		
def extractTar(ID,directory):
	tarName=ID[ID.rfind('LC8'):]
	# tar = tarfile.open(os.path.join(directory,ID,tarName + ".tgz"))
	# tar.extractall(directory)
	# tar.close()
	return(tarName)

#repID,rojects a raster (src) to another raster's specifications (match_ds) and outputs the reprojected raster (dst_filename)
...
#Path for your area needs to be defined
def main():
