...
import gdal, ogr, os, osr,sys,datetime,jdcal,math,tarfile,glob,copy,time,warnings,distutils
import numpy as np
...
def cropRastersToArrays(minExtent,pixelX,pixelY,inputRasters):
	for band in inputRasters.keys():
		...
		inputRasters[band]=inputRasters[band].ReadAsArray(int(pixels[0]),(int(pixels[3])-1),int(pixels[1]),int(pixels[2]))
	return(inputRasters)
...
def slipCompare(path,row,todayExtent,date):
	...
	slopemask=dict([])
	slopemask['mask']=gdal.Open(os.path.join('DEM','fake.TIF'))
	...
	if np.sum(finalDetection[finalDetection==3])>0: 
		...
		oldFiles=sorted(glob.glob(os.path.join(getCurrentDirectory(),'Today',path,row,'today' + '*.TIF')))
		print(oldFiles)
