...
def sizeof_fmt(num):
    for x in ['bytes', 'KB', 'MB', 'GB', 'TB']:
        ...
        num /= 1024.0 
#############################
def downloadChunks(url,rep,nom_fic,usgs,proxy_info):
  ...
  try:
    ...
    if (req.info().gettype()=='text/html'):
        ...
        if lines.find('Download Not Found')>0 :
	        ...
	        with open("error_output.html","w") as f: f.write(lines)
...
def next_overpass(date1,path,sat):
    ...
    """
    date0_L5 = datetime.datetime(2010,5,4)
    date0_L7 = datetime.datetime(2013,1,11)
    date0_L8 = datetime.datetime(2016,5,1)
    if sat=='LT5':
        date0=date0_L5
    elif sat=='LE7':
        date0=date0_L7
...
def getmetadatafiles(destdir,option):
    ...
    links=['LANDSAT_8.csv','LANDSAT_ETM.csv','LANDSAT_ETM_SLC_OFF.csv','LANDSAT_TM-2007-2010.csv','LANDSAT_TM-2011-2013.csv','LANDSAT_TM-2014-2016.csv']
    for l in links:
                ...
                for version in ['00','01','02']:			
					...
					if os.path.exists(lsdestdir):
						...
						try:
							...
							downloadChunks(url,"%s"%rep_scene,nom_prod+'.tgz',usgs,proxy)
						except:
